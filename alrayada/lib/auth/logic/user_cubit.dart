...
class UserCubit extends Cubit<UserState> {
  UserCubit({required this.userApi}) : super(const UserInitial()) {
    ...
    // Updates the state when the API logs out the user (e.g., user account removed or expired token)
    // Will be required in case the user is calling an action that doesn't
    // handle logout.
    userApi.setOnAutoLogout(() {
      // TODO: Test this, we might not need the conditional check
      print('On Auto Logout: $state');
  ...
  Future<void> fetchUser() async {
    ...
    try {
      ...
      if (user == null) {
        print("You're no longer authenticated in User Cubit");
        // The user has is no longer logged in (e.g., user account removed or expired token)
        await logout();
        return;
      }
  ...
  Future<void> logout() async {
    try {
      ...
      // TODO: Handle this error, might need to refactor the state to have UserLogoutInProgress etc...
      AppLogger.error(
  ...
  Future<void> updateUserInfo(
    ...
    } on InvalidCredentialsUserException {
      // TODO: It's throwing out UnknownUserException instead of UserNotLoggedInAnyMoreUserException
    } on UserException catch (e) {
